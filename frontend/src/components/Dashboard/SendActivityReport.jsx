...
const SendActivityReport = ({setOpenEmailForm, openEmailForm, setEmailAssignmentId, emailAssignmentId, setCustomerId, customerId}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(async () => {
      ...
      formData.append("task_update", pdfFile);
      ...
      try {
        const response = await axiosPrivate.post(
          `/workflow/send-client-work/task-status/${emailAssignmentId}/`,
          formData,
          {
            headers: {
    ...
    const fetchClient = async () => {
      try {
        const response = await axiosPrivate.get(`/clients/get-customer/${customerId}/`);
  ...
  return (
    <div className="w-[50%] mx-auto  p-4 bg-white rounded shadow-2xl font-poppins">
        ...
        <h2 className="text-xl font-semibold mb-4">Send Activity Report (PDF)</h2>
            <X onClick={()=>{setEmailAssignmentId();setOpenEmailForm(false)}} />
        </div>
...
export default SendActivityReport;
