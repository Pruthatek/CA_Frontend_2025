...
const AddOutward2 = ({ setCreateOutward, createOutward, setSelectedOutward, fetchOutwards, selectedOutward }) => {
  const navigate = useNavigate();
  const { selectedColor } = useColor();
  const axiosPrivate = useAxiosPrivate();

     const [isEditMode, setIsEditMode] = useState(!!selectedOutward);
    const [editingId, setEditingId] = useState(selectedOutward ? selectedOutward.id : null);

  const [company, setCompany] = useState(selectedOutward?.company || '');
  const [customer, setCustomer] = useState(selectedOutward?.customer || '');
  const [outwardReference, setOutwardReference] = useState(selectedOutward?.outward_reference || '');
  const [inward, setInward] = useState(selectedOutward?.inward || '');
  const [outwardTitle, setOutwardTitle] = useState(selectedOutward?.outward_title || '');
  const [aboutOutward, setAboutOutward] = useState(selectedOutward?.about_outward || '');
  const [outwardThrough, setOutwardThrough] = useState(selectedOutward?.outward_through || '');
  const [outwardDate, setOutwardDate] = useState(selectedOutward?.outward_date || '');
  const [avbNo, setAvbNo] = useState(selectedOutward?.avb_no || '');
  const [courierName, setCourierName] = useState(selectedOutward?.courier_name || '');
  const [nameOfPerson, setNameOfPerson] = useState(selectedOutward?.name_of_person || '');
...
const handleSubmit = async (e) => {
    ...
    try {
        if (isEditMode) {
          await axiosPrivate.put(`/documents/outward/update/${editingId}/`, formData);
      ...
      try {
        await fetchOutwards(); // Fetch updated data
                 
        setCreateOutward(false); // Close modal or UI update
  ...
  return (
    <div className="w-[100%] relative rounded-[10px] bg-white overflow-x-scroll flex xl:flex-row flex-col px-3 gap-4 h-[600px] overflow-y-scroll pb-20 no-scrollbar font-poppins">
      <div className="absolute right-3 top-3 flex items-center gap-x-3">
        ...
        <X onClick={() => {setSelectedOutward(null); setCreateOutward(false)}} className="cursor-pointer" />
      </div>
