...
const SubmitClientWork = ({ setOpenSubmit, selectedAssignment }) => {
  ...
  const fetchById = async () => {
    try {
      const response = await axiosPrivate.get(
        `/workflow/client-work-category-assignment/get/${selectedAssignment.id}/`
      );
  ...
  const submitNewActivity = async () => {
    ...
    try {
      ...
      alert(
        "Additional activity added successfully with id: " + response.data.id
      );
  ...
  const submitNewAdditionalFile = async () => {
    ...
    try {
      ...
      alert("Additional file added successfully with id: " + response.data.id);
      // Clear form fields and refresh data if needed
    ...
    <div className="p-6 w-[60%] h-[500px] overflow-y-scroll mx-auto bg-white shadow-2xl border rounded-lg space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 ">
          ...
          {activities.length > 0 ? (
            activities.map((activity, index) => {
              ...
              return (
                <div
                  key={activity.id}
                  className="flex flex-col gap-y-2 p-2 border rounded"
                >
                  <div className="flex gap-x-4 items-center text-[12px]">
                    <p>
                      <b className="font-semibold">ID:</b> {activity.id}
                    </p>
                    <p>
                      <b className="font-semibold">Activity:</b>{" "}
                      {activity.activity}
                    </p>
                    <p>
                      <b className="font-semibold">Assigned %:</b>{" "}
                      {activity.assigned_percentage}%
                    </p>
                  </div>
                  ...
                  <div className="flex gap-x-4 items-center text-[12px]">
                    <label htmlFor={`note-${activity.id}`} className="font-semibold">
                      Note:
                    </label>
                    <input
                      id={`note-${activity.id}`}
          ...
          {activityStages.length > 0 ? (
            activityStages.map((activity, index) => {
              ...
              return (
                <div
                  key={activity.id}
                  className="flex flex-col gap-y-2 p-2 border rounded text-[12px]"
                >
                  <div className="flex gap-x-4 items-center">
                    ...
                    <p>
                      <b className="font-semibold">Activity Stage:</b>{" "}
                      {activity.activity_stage}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-x-4 items-center">
                    <label htmlFor={`note-${activity.id}`} className="font-semibold text-[12px]">
                      Note:
                    </label>
                    <input
                      id={`note-${activity.id}`}
                      ...
                      onChange={(e) => {
                        const newStages = [...activityStages];
                        newStages[index].note = e.target.value;
                        setActivityStages(newStages);
                      }}
                      className="border border-[#383a3e] rounded-[4px]"
                    />
          ...
          {outputFiles.length > 0 ? (
            outputFiles.map((file, index) => (
              ...
              >
                <div className="flex gap-x-4 items-center text-[12px]">
                  <p>
                    <b className="font-semibold">ID:</b> {file.id}
                  </p>
                  <p>
                    <b className="font-semibold">File Name:</b> {file.file_name}
                  </p>
                  <p>
                    <b className="font-semibold">Current Path:</b> {file.file_path}
                  </p>
                </div>
                <input
                  type="file"
          ...
          {requiredFiles.length > 0 ? (
            requiredFiles.map((file, index) => (
              ...
              >
                ...
                {/* 
                  If you need to enable updating the required file, 
        ...
        <div className="border p-4 rounded-lg">
          ...
          />
          {/* 
            Now that we only have "completed" / "pending", 
            we can hardcode newActivity.status as "pending" by default 
          */}
          <input
            type="text"
            placeholder="Note"
